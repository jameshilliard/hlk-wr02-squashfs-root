...
function initTranslation()
{
	...
	e = document.getElementById("APOnOffs");
	e.innerHTML = _("basic APOnOffs");
...
//	if(((opmode == '0')&&(apcli_en == '1'))
//		||((opmode == '3'))){
	if(1){
	}
	else{
		document.getElementById("APOnOffsDiv").style.visibility = "hidden";
		document.getElementById("APOnOffsDiv").style.display = "none";
	}

//	if('0' == '<% getCfgZero(1, "HLK_APOnOff"); %>'){
	if('0' == '<% getCfgGeneral(1, "HLK_APOnOff"); %>'){
		
		document.getElementById("APOnOff").options.selectedIndex = 0 ;
	}
	else{
		document.getElementById("APOnOff").options.selectedIndex = 1 ;
	}
...
<table width="540" border="1" cellspacing="1" cellpadding="3" bordercolor="#9BABBD">
  ...
  <tbody id="APOnOffsDiv">
  <tr>
    <td class="head" id="APOnOffs">AP On/Off</td>
    <td>
      <select name="APOnOff" id="APOnOff" size="1" >
        <option value=0>AP Off</option>
        <option value=1>AP On</option>
      </select>
    </td>
  </tr>
  </tbody>
