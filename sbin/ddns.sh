...
if [ "$srv" = "dyndns.org" ]; then
	...
	inadyn -u $u -p $pw -a $ddns --dyndns_system default@$srv &
elif [ "$srv" = "oray.com" ]; then
	inadyn -u $u -p $pw -a $ddns --dyndns_server_name ddns.oray.com:80 --dyndns_system custom@http_svr_basic_auth --dyndns_server_url /ph/update?hostname=  --ip_server_name ddns.oray.com /checkip &
else
